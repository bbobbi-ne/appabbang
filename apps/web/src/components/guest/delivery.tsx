...
import { useGetGuestOrderDeliveryQuery } from '@/hooks/use-guest';
...
export default function GuestDeliveryDetail({ orderNo }: Props) {
  const navigate = useNavigate();
  const { data: order, isLoading } = useGetGuestOrderDeliveryQuery(orderNo);
  ...
  /** 비회원 주문 상세내역 이동 */
  const moveToOrderDetail = (no: number) => navigate({ to: `/guest/order-list/${no}` });

  useEffect(() => {
    if (order) {
      if (!AVALIABLE_DELIVERY_ORDER_STATUS.includes(order.orderStatus!)) {
        navigate({ to: `/guest/order-list/${order.no}` });
      }
    }
  }, [order]);
